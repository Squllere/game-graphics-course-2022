...
import {positions, normals, indices} from "../blender/pontsik.js"
import {positions as planePositions, uvs as planeUvs, indices as planeIndices} from "../blender/plane.js"
import {positions as secondPositions, uvs as secondUvs, indices as secondIndices} from "../blender/cube.js"
...
let lightCalculationShader = `
    ...
    vec4 calculateLights(vec3 normal, vec3 position) {
        ...
        for (int i = 0; i < lightPositions.length(); i++) {
            ...
            float specular = pow(max(dot(normalize(lightDirection + viewDirection), normal), 0.2), specularPower);
...
let fragmentShader = `
    ...
    void main()
    {       
        vec3 shadowCoord = (vPositionFromLight.xyz / vPositionFromLight.w) / 9.0 + 0.5;        
...
let mirrorFragmentShader = `
    ...
    {                        
        ...
        screenPos.x += (texture(distortionMap, vUv).r - 0.8) * 0.2;
...
mat4.fromXRotation(modelMatrix, -Math.PI / 8);
...
function draw(timems) {
    ...
    for (let i = 0; i < numberOfPointLights; i++) {
        vec3.rotateZ(pointLightPositions[i], pointLightInitialPositions[i], vec3.fromValues(10, 0, 0), time);
        positionsBuffer.set(pointLightPositions[i], i * 2);
