...
import {positions, normals, uvs, indices} from "../blender/lampjalg.js"
...
// let kinnitab teatud numbri uhiku uhte plokki et mujalt teda kasutada ei saaks
...
//kasitleb  ja kujutab ekraanil olevaid alasi mida peab....looma?
...
let ambientLightColor = vec3.fromValues(.9, .1, .4);
let lightColors = [vec3.fromValues(.95, .7, .95), vec3.fromValues(.6, .9, .9)];
let lightInitialPositions = [vec3.fromValues(20, 0, 100), vec3.fromValues(100, 0, 100)];
...
let mirrorVertexShader = `
    ...
    out vec2 vUv;
        //alus peegli moonutamine
    void main()
    {
        vUv = uv;
        gl_Position = (modelViewProjectionMatrix * vec4((position + vec3(0.0, -0.4, 0.0)), .015));
    }
`;
...
let skyboxFragmentShader = `
    ...
    void main() {
      ...
      outColor = texture(cubemap, normalize(t.xyz / t.w)) * vec4(1, time, .1, .1);
    }
`;
...
(async () => {
    const cubemap = app.createCubemap({
        negX: await loadTexture("kuu.jpg"),
    posX: await loadTexture("kuu.jpg"),
    negY: await loadTexture("kuu.jpg"),
    posY: await loadTexture("kuu.jpg"),
    negZ: await loadTexture("kuu.jpg"),
    posZ: await loadTexture("kuu.jpg")
    });
    ...
    let mirrorDrawCall = app.createDrawCall(mirrorProgram, mirrorArray)
        ...
        .texture("distortionMap", app.createTexture2D(await loadTexture("pattern.png")));
    ...
    function draw() {
        ...
        //kaamera sisse ja valjs kissitamine
        mat4.perspective(projMatrix, Math.PI / 2, app.width / app.height, 0.1, 400.0);
        vec3.rotateY(cameraPosition, vec3.fromValues(clamp(Math.abs(Math.sin(time * .2) * 10), 60, 120 ), 10 + 1 * Math.sin(time * .4), 0), vec3.fromValues(0, 0, 0), time * .1);
        ...
        app.viewport(2, 0, colorTarget.width, colorTarget.height);
